...
export class DigiKabuAPI extends Logging {
    ...
    async handle_fetch_errors<t>(result: Result<t, { type: "status", code: number, text: () => Promise<string> } | { type: "json", err: any, rawText: string } | { type: "html", err: any, rawText: string } | { type: "zod", err: any, rawObject: any }>) {
        if (result.error) {
            switch (result.error.type) {
                case "status": throw new Error("Status was not 200! Status code: " + result.error.code + " Message: " + await result.error.text());
                case "json": throw new Error("Error while parsing response as json:\nResponse: " + result.error.rawText + "\nCatched error: " + result.error.err);
                case "zod": throw new Error("Error while verifying object:\nObject: " + result.error.rawObject + "\nCatched error: " + result.error.err);
    ...
    >(
        ...
        { type: "status", code: number, text: () => Promise<string> }
    ...
    >> {
        ...
        if (rawResp.status != 200) return {
            error: {
                type: "status",
                code: rawResp.status, 
                text: () => rawResp.text().catch(err => "Error while getting message text: " + err)
            }
        }
